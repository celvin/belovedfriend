...
function buildStorageOptions(): StorageOptions {
  ...
  if (inline) {
    const credentials = JSON.parse(inline) as {
      project_id?: string;
      client_email: string;
      private_key: string;
    };
  ...
  // file path, gcloud auth, or workload identity).
  return {};
}
...
export class ObjectStorageService {
  ...
  getPublicObjectSearchPaths(): Array<string> {
    ...
    if (paths.length === 0) {
      throw new Error(
        "PUBLIC_OBJECT_SEARCH_PATHS not set (comma-separated /bucket/prefix list).",
      );
    }
    return paths;
  }
  ...
  getPrivateObjectDir(): string {
    ...
    if (!dir) {
      throw new Error("PRIVATE_OBJECT_DIR not set (e.g. /bucket/private).");
    }
    return dir;
  }
  ...
  normalizeObjectEntityPath(rawPath: string): string {
    // Signed URLs from getSignedUrl() are https://storage.googleapis.com/<bucket>/<object>?...
